...
// Offline replay: re-decode an existing capture (c2s.bin/s2c.bin + meta.txt) through RdpSession and
// rewrite pdus.log/pdus.jsonl. Lets us iterate on the decoders without a live re-capture.
//   rdpmitm --replay [dumpDir]
if (args.Length >= 1 && args[0] == "--replay")
{
    string rdir = args.Length > 1 ? args[1] : "/tmp/rdpmitm";
    return ReplayDump(rdir);
}

if (args.Length < 5)
{
    Console.Error.WriteLine("usage: rdpmitm <listenPort> <host> <hostPort> <user> <password> [domain] [dumpDir]");
    Console.Error.WriteLine("   or: rdpmitm --replay [dumpDir]");
    return 1;
}
...
async Task HandleAsync(TcpClient mstscTcp)
{
    ...
    // Structural decoder: receive -> decode -> log -> re-encode -> (forward re-encoded iff byte-identical).
    // One session shared by both pump directions so server-learned channel/DVC/GFX state labels client
    // traffic too. Output: <dumpDir>/pdus.log (readable) + pdus.jsonl (machine).
    var sink = new RdpLogSink(dumpDir);
    var rdp = new RdpSession(sink);
    ...
    try
    {
        ...
        var t1 = Pump(mstscSsl, hostSsl, c2s, meta, "C2S", swStart, selected, false, rdp, RdpDir.ClientToServer);        // mstsc->host: patch CS_CORE.serverSelectedProtocol
        var t2 = Pump(hostSsl, mstscSsl, s2c, meta, "S2C", swStart, mstscRequested, true, rdp, RdpDir.ServerToClient);   // host->mstsc: patch SC_CORE.clientRequestedProtocols
    ...
    finally
    {
        ...
        Console.WriteLine($"[{id}] decoded {sink.Count} PDUs, {sink.Mismatches} round-trip mismatches");
        sink.Dispose();
        mstscTcp.Dispose();
        Console.WriteLine($"[{id}] session closed (dumps + pdus.log/pdus.jsonl written)");
    }
}
    ...
    RdpSession rdp, RdpDir rdpDir)
{
    ...
    try
    {
        while (true)
        {
            ...
            // Patch BEFORE decode/forward so the structural log and the round-trip reflect the bytes we
            ...
            if (patchProtocol is uint val && !patched)
            {
                ...
                {
                    patched = true;
                    rdp.NotePatch(rdpDir, isServerData ? "clientRequestedProtocols" : "serverSelectedProtocol", val);
            ...
            // receive -> decode -> log -> re-encode -> forward (re-encoded iff byte-identical, else
            // the original bytes so the live session is never broken).
            byte[] forward = rdp.Feed(rdpDir, buf.AsSpan(0, n));
            await to.WriteAsync(forward);
            await to.FlushAsync();
        }
    }
    catch { /* peer closed */ }
...
// Re-decode an existing dump in true wire order (from meta.txt), rewriting pdus.log/pdus.jsonl.
static int ReplayDump(string dir)
{
    // Accept either the MITM dump (c2s.bin/s2c.bin + meta.txt for true wire ordering) or the gateway
    // RDPGW_DUMP_DIR tee (our_c2s.bin/our_s2c.bin, no meta — replay s2c then c2s as whole streams).
    string metaPath = Path.Combine(dir, "meta.txt");
    string c2sPath = File.Exists(Path.Combine(dir, "c2s.bin")) ? Path.Combine(dir, "c2s.bin")
                                                               : Path.Combine(dir, "our_c2s.bin");
    string s2cPath = File.Exists(Path.Combine(dir, "s2c.bin")) ? Path.Combine(dir, "s2c.bin")
                                                               : Path.Combine(dir, "our_s2c.bin");
    if (!File.Exists(c2sPath) || !File.Exists(s2cPath))
    {
        Console.Error.WriteLine($"replay: need (c2s.bin|our_c2s.bin) + (s2c.bin|our_s2c.bin) in {dir}");
        return 1;
    }

    var c2s = File.ReadAllBytes(c2sPath);
    var s2c = File.ReadAllBytes(s2cPath);
    using var sink = new RdpLogSink(dir, echoConsole: false);
    var rdp = new RdpSession(sink);

    if (File.Exists(metaPath))
    {
        // True wire order from meta.txt: "<ms> C2S <n>" / "<ms> S2C <n>" (PATCH lines have no byte count).
        int c2sOff = 0, s2cOff = 0, units = 0;
        foreach (var line in File.ReadLines(metaPath))
        {
            var parts = line.Split(' ', StringSplitOptions.RemoveEmptyEntries);
            if (parts.Length < 3) continue;
            string d = parts[1];
            if (d != "C2S" && d != "S2C") continue;
            if (!int.TryParse(parts[2], out int n) || n <= 0) continue;
            if (d == "C2S")
            {
                int take = Math.Min(n, c2s.Length - c2sOff);
                if (take <= 0) continue;
                rdp.Feed(RdpDir.ClientToServer, c2s.AsSpan(c2sOff, take));
                c2sOff += take;
            }
            else
            {
                int take = Math.Min(n, s2c.Length - s2cOff);
                if (take <= 0) continue;
                rdp.Feed(RdpDir.ServerToClient, s2c.AsSpan(s2cOff, take));
                s2cOff += take;
            }
            units++;
        }
        Console.WriteLine($"replay: fed {units} chunks (c2s {c2sOff}/{c2s.Length}B, s2c {s2cOff}/{s2c.Length}B) -> {dir}/pdus.log");
    }
    else
    {
        // No meta: feed c2s first so the channel map is learned from CS_NET (the client's static-channel
        // names) before the s2c stream's DVC/GFX data is decoded — otherwise drdynvc traffic would be
        // misrouted to the I/O share channel (logged as share.unknown). SC_NET ids match positionally and
        // DVC CREATEs precede GFX DATA within s2c, so a single s2c pass then resolves everything.
        rdp.Feed(RdpDir.ClientToServer, c2s);
        rdp.Feed(RdpDir.ServerToClient, s2c);
        Console.WriteLine($"replay: fed c2s {c2s.Length}B + s2c {s2c.Length}B (no meta) -> {dir}/pdus.log");
    }
    return 0;
}
